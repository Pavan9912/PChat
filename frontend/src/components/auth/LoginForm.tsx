...
export const LoginForm: React.FC = () => {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const res = await fetch(`${apiHost}/api/auth/login-otp`, {
        method: 'POST',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (useOtpLogin) {
      ...
      dispatch(authStart());
      try {
        const res = await fetch(`${apiHost}/api/auth/login-otp-verify`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    <div className="w-full max-w-md bg-dark-panel border border-neutral-800 p-8 rounded-2xl shadow-2xl relative z-10">
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-dark-secondary">
          {useOtpLogin ? 'Sign in passwordless using email OTP.' : 'Please sign in to access your chat feed.'}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="text-center mt-4">
          ...
          >
            {useOtpLogin ? 'Sign In with Password' : 'Sign In with Email OTP'}
          </button>
        </div>

        {useOtpLogin && otpSent && (
          <div className="flex justify-between items-center mt-4 text-xs text-dark-secondary px-1">
            <button
              type="button"
